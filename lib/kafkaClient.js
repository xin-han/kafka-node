...
KafkaClient.prototype.wrapTimeoutIfNeeded = function (socket, correlationId, callback, overrideTimeout) {
  ...
  timeoutId = setTimeout(() => {
    this.unqueueCallback(socket.socketId, correlationId);
    callback(new TimeoutError(`Request timed out after ${timeout}ms. CorrelationId: ${correlationId}, socketId: ${socket.socketId}, brokerHost: ${socket.host}, brokerPort: ${socket.port}, localPort: ${socket.localPort}, bytesRead: ${socket.bytesRead}`));
    callback = _.noop;
  }, timeout);
...
KafkaClient.prototype.queueCallback = function (socket, id, data) {
  data[1] = this.wrapTimeoutIfNeeded(socket, id, data[1], data[2]);
...
KafkaClient.prototype.waitUntilReady = function (broker, callback) {
  ...
  if (timeout !== false) {
    timeoutId = this._createTimeout(() => {
      ...
      const brokerSocketInfo = broker.socket ? `SocketId: ${broker.socket.socketId}, brokerHost: ${broker.socket.host}, brokerPort: ${broker.socket.port}` : ''
      callback(new TimeoutError(`Request timed out after ${timeout}ms. ${brokerSocketInfo} waitUntilReady`));
    }, timeout);
  }
